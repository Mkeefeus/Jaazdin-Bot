...
export async function execute(interaction: ChatInputCommandInteraction) {
  ...
  Object.keys(categories).forEach((category) => {
    ...
    if (ingredientsInCategory && ingredientsInCategory.length > 0) {
      const randomIndex = Math.floor(Math.random() * ingredientsInCategory.length);
      sandwich[category as IngredientCategory] = ingredientsInCategory[randomIndex].name;
    }
  });
  ...
  const embed = new EmbedBuilder()
    ...
    .setColor(sandwichScore >= 95 ? 0xffd700 : sandwichScore >= 90 ? 0x00ff00 : 0x0099ff)
    ...
    .addFields(
      ...
      },
      {
        ...
        value: `${sandwichScore}/100 - ${
          sandwichScore >= 95 ? 'LEGENDARY!' : sandwichScore >= 90 ? 'EPIC!' : 'AWESOME!'
        }`,
        inline: false,
      }
    )
